...
export default function Page() {
  ...
  async function saveDoc(type: AddMode, data: Record<string, any>) {
    ...
    try {
      ...
      const savedDate = data.date || datePart(data.startAt) || data.dueAt;
      if (savedDate) {
        setSelectedDate(String(savedDate).slice(0, 10));
        setCalendarMonth(String(savedDate).slice(0, 10));
      }
      setOperationMessage(editTarget ? '更新しました' : '保存しました');
...
function CalendarHomeView({ selectedDate, setSelectedDate, chooseDate, calendarMonth, setCalendarMonth, diaries, events, todos, expenses, anniversaries, monthExpense, openAdd, onEdit, onDelete, currentUserId, naturalAdd, setTab, saving, spaces, groupId, viewAllSpaces, activeSpaceIds, switchSpace, switchAllSpaces }: any) {
  ...
  const submitNatural = () => {
    ...
    naturalAdd(naturalText);
    setNaturalText('');
  };

  return <><section className="home-summary">
    <button className="summary-tile primary" onClick={() => { const today = todayIso(); setSelectedDate(today); setCalendarMonth(today); }}><span>今日</span><b>{todayIso().slice(5).replace('-', '/')}</b></button>
    <button className="summary-tile" onClick={() => setTab('todo')}><span>未完了</span><b>{todos.filter((t: Todo) => t.status === 'open').length}</b></button>
    ...
    <section className="calendar-card">
      ...
      <div className="month-grid">{cells.map((day, i) => {
        ...
        return <button key={date} className={`day-cell ${date === selectedDate ? 'selected' : ''} ${date === todayIso() ? 'today' : ''}`} onClick={() => chooseDate(date)} onContextMenu={e => { e.preventDefault(); openAdd('event', date); }}><span>{day}</span><div className="marks">{marks.event && <i className="event" />}{marks.todo && <i className="todo" />}{marks.diary && <i className="diary" />}{marks.expense && <i className="expense" />}{marks.anniversary && <i className="anniv" />}</div></button>;
      })}</div>
    </section>
    <section className="selected-day-panel">
      ...
      <div className="day-list">
        ...
        {!hasSelectedItems && <div className="empty-state"><b>この日はまだ空いています</b><p>日記、予定、ToDo、支出をこの日付に追加できます。</p><div className="empty-actions"><button className="btn secondary" onClick={() => openAdd('event', selectedDate)}>予定</button><button className="btn secondary" onClick={() => openAdd('todo', selectedDate)}>ToDo</button></div></div>}
      </div>
    </section>
  </>;
}
...
function DateActionSheet({ selectedDate, close, openAdd }: { selectedDate: string; close: () => void; openAdd: (mode: AddMode, date?: string) => void }) {
  return <div className="date-sheet-backdrop" onClick={close}><div className="date-sheet" onClick={e => e.stopPropagation()}><div className="row"><div><p className="eyebrow">追加先を選択</p><h3>{new Date(`${selectedDate}T00:00:00+09:00`).toLocaleDateString('ja-JP', { dateStyle: 'full' })}</h3></div><button className="icon-btn" onClick={close}>×</button></div><div className="date-sheet-actions list"><button onClick={() => openAdd('diary', selectedDate)}><span><NotebookPen size={19} /></span><b>日記を書く</b><small>その日の出来事や気分を残す</small></button><button onClick={() => openAdd('todo', selectedDate)}><span><CheckSquare size={19} /></span><b>ToDoを追加</b><small>期限付きのやることを登録</small></button><button onClick={() => openAdd('event', selectedDate)}><span><CalendarDays size={19} /></span><b>予定を追加</b><small>開始時刻やリマインドを設定</small></button><button onClick={() => openAdd('expense', selectedDate)}><span><ReceiptText size={19} /></span><b>支出を追加</b><small>金額やカテゴリを記録</small></button></div></div></div>;
}
...
function SettingsView({ user, groupId, shareEnabled, activeSpaceName, spaces, spaceMembers, setGroupId, createSpace, renameSpace, leaveSpace, createInvite, revokeInvite, joinInvite, saveMemberProfile, reload, notificationEnabled, setNotificationEnabled, pushStatus, enablePush, repairSearchIndex, saving, sharedStats }: any) {
  ...
  const notificationLabel = permission === 'granted' ? '許可済み' : permission === 'denied' ? '拒否中' : permission === 'unsupported' ? '未対応' : '未許可';
  ...
  return <Section title="設定">
    ...
    <div className="settings-group-title">参照スペース</div>
    <div className="card line-settings-card"><div className="line-card-head"><h3><Users size={18}/> スペース一覧</h3><button className="btn secondary" disabled={saving} onClick={() => setShareMode('invite')}><Share2 size={16}/> 新規作成</button></div><p className="muted section-note">AIチャットbotが参照する個人・共有グループを選びます。表示の切替はホーム画面でできます。</p><div className="line-thread-list">{spaceOptions.map((space: SpaceSummary) => {
    ...
    <div className="settings-group-title">選択中スペース</div>
    ...
    <div className="card settings-list-card"><h3><Bell size={18}/> 通知</h3><div className="notification-status"><span>ブラウザ通知</span><b>{notificationLabel}</b><small>{standalone ? 'ホーム画面アプリとして起動中' : 'ブラウザで起動中'}</small></div><label className="settings-row"><span>アプリ起動中の通知チェック</span><input type="checkbox" checked={notificationEnabled} onChange={e => setNotificationEnabled(e.target.checked)} /></label><button className="btn" onClick={enablePush}>PWA Push通知を有効化</button><p className="muted">状態: {pushStatus}</p><p className="muted">iPhoneはSafariで開く → 共有 → ホーム画面に追加 → 追加したアイコンから開いて通知許可、の順に設定してください。</p></div>
    <div className="settings-group-title">メンテナンス</div>
    <div className="card settings-list-card"><h3>AI検索</h3><p className="muted">表示対象のAI検索だけを修復します。単一スペース表示中はそのスペースのみ、「すべて」表示中は参加中スペース全体が対象です。</p><button className="btn secondary" disabled={saving} onClick={repairSearchIndex}>{saving ? '修復中...' : '表示対象のAI検索を修復'}</button></div>
